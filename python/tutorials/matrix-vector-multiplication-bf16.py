...
LINE_VALS = [
    'triton-cpu-single', 'triton-cpu', 'torch-cpu-compile-single', 'torch-cpu-compile'
]
LINE_NAMES = [
    'TritonCPU 1', 'TritonCPU', 'TorchCPU (compile) 1', 'TorchCPU (compile)'
]
...
def benchmark(M, N, provider):
    ...
    if 'torch-cpu-compile' in provider:
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: compiled_matmul(weight, x, out=output), quantiles=quantiles,
                                                     is_cpu=True)
