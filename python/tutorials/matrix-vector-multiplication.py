...
def gemv(
    ...
    if output is None:
        ...
        assert output.shape == (M, ) and output.dtype == torch.float32, "Incompatible output"
...
torch_output = torch.matmul(weight, x[:, None]).reshape(-1).to(torch.float32)
...
LINE_NAMES = [
    'TritonCPU 1', 'TritonCPU', 'TorchCPU (native)', 'TorchCPU (compile)'
]
