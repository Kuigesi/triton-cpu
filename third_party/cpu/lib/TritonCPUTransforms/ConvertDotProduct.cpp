...
struct ConvertContracToDot
  ...
  LogicalResult matchAndRewrite(vector::ContractionOp op,
    ...
    const int res_lanes = vec_length / (resTy.getElementType().getIntOrFloatBitWidth());

    Type vecResTy = VectorType::get(res_lanes, resTy.getElementType());
    ...
    for (int64_t idx = 0; idx < num_regs; idx += 1) {
