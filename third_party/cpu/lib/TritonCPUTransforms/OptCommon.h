...
inline Value shapeCast(Location loc, Value in, VectorType outTy, PatternRewriter &rewriter) {
...
inline Value shapeCast(Location loc, Value in, std::initializer_list<int64_t> shapes, PatternRewriter &rewriter) {
  VectorType inTy = cast<VectorType>(in.getType());
